...
app.face.start = (videoId, callback) => {
    ...
    tracker.on('track', function(event) {
        ...
        data.forEach(function(rect, index) {
            var x = canvas.width - rect.x - rect.width/2 + 20; // 20 calibration
            var y = rect.y + rect.height/2 + 3 ; // 3 calibration
            printFacePosition(canvas, x, y, index);             
            
            callback(generateRelativePosition(x, y, app.dimentions));
        });
    });
};

function printFacePosition(canvas, x,y, index) {
    var context = canvas.getContext("2d");
    context.strokeStyle = index === 0 ? 'red' : index === 1 ? 'blue' : 'green';
    context.beginPath();
